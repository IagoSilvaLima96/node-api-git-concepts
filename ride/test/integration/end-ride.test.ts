...
import { EndRide } from "../../src/application/usecase/end-ride";
...
import { PaymentHttpGateway } from "../../src/infra/gateway/payment-http.gateway";
...
test("Deve finalizar uma corrida", async function () {
  ...
  const inputEndRide = {
    rideId: outputRequestRide.rideId,
    date: new Date("2023-03-01T10:40:00"),
  };
  const paymentGateway = new PaymentHttpGateway(httpClient);
  const endRide = new EndRide(
    rideDatabaseRepository,
    accountGateway,
    paymentGateway
  );
  ...
  expect(outputGetRide.endDate).toEqual(inputEndRide.date);
});
